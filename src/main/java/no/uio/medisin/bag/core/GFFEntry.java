...
public class GFFEntry implements Comparable<GFFEntry>{
    ...
    public int compareTo(GFFEntry queryGFFEntry){
        ...
        if (queryGFFEntry.refSeqID.equals(refSeqID) && queryGFFEntry.strand == strand)
        {
            return start - queryGFFEntry.start ;
        }
        
        if(queryGFFEntry.strand != strand){
            if(strand == Strand.PLUS && queryGFFEntry.strand == Strand.MINUS){
                return -1;
            }else{
                return +1;
            }            
